...
class Model_ACTV2Config(ReasoningModelConfig):
    # Shares the common config in models/config.py (which provides `causal` and
    # the rest). This is a pure-transformer baseline with no L-level recursion,
    # so the base class's required L_cycles/L_layers are unused here and default
    # to 0. act_enabled/act_inference are specific to this baseline.
    L_cycles: int = 0
    L_layers: int = 0
